...
QString MacHostsManager::hostsFilePath() const
{
    return QStringLiteral("/etc/hosts");
}

QStringList MacHostsManager::readHostsFile() const
{
...
bool MacHostsManager::writeHostsFile(const QStringList& lines) const
{
    QFile file(hostsFilePath());
...
bool MacHostsManager::addEntry(const QString& ip, const QString& domain)
{
...
bool MacHostsManager::removeEntry(const QString& domain)
{
...
bool MacHostsManager::hasEntry(const QString& domain)
{
    QStringList lines = readHostsFile();
...
QList<QPair<QString, QString>> MacHostsManager::listEntries()
{
...
bool MacHostsManager::flush()
{
    LOG_INFO("Flushing DNS cache (dscacheutil + mDNSResponder)");

    // Flush the directory service cache
    QProcess dscache;
    dscache.start("dscacheutil", {"-flushcache"});
    if (!dscache.waitForStarted(5000)) {
        LOG_ERROR("Failed to start dscacheutil");
        return false;
    }
    if (!dscache.waitForFinished(10000)) {
        LOG_ERROR("dscacheutil timed out");
        dscache.kill();
        return false;
    }
    if (dscache.exitCode() != 0) {
        LOG_WARNING(QString("dscacheutil failed (exit %1)").arg(dscache.exitCode()));
    }

    // Send HUP signal to mDNSResponder to flush its cache
    QProcess killall;
    killall.start("killall", {"-HUP", "mDNSResponder"});
    if (!killall.waitForStarted(5000)) {
        LOG_ERROR("Failed to start killall for mDNSResponder");
        return false;
    }
    if (!killall.waitForFinished(10000)) {
        LOG_ERROR("killall mDNSResponder timed out");
        killall.kill();
        return false;
    }
    if (killall.exitCode() != 0) {
        QString stdErr = QString::fromLocal8Bit(killall.readAllStandardError());
        LOG_WARNING(QString("killall -HUP mDNSResponder failed (exit %1): %2")
                        .arg(killall.exitCode()).arg(stdErr));
    }
